...
<html lang="en">
<head>
	<title>DKSTORE - QUẢN LÝ TÀI KHOẢN</title>
	<meta charset="UTF-8">
			...
			<div class="row">
				<div class="col-sm-10 col-lg-7 col-xl-3 m-lr-auto m-b-50 mt-2 bor14">
					<div class="bor10 m-r-40 m-lr-0-xl bor14">
						<div class="list-group">
							<a href="?ping=1" class="list-group-item bg6 list-group-item-action">Xuất mã đăng phập</a>
				...
				<?php
						...
						if(!isset($_REQUEST['ping'])||$_REQUEST['ping']==2){
									...
									case 'update':
										{
											...
											if($HoDem!=''&&$Ten!=''&&$DiaChi!=''&&$Phone!=''&&$Email!=''){
												...
												$read->readCustomer("http://localhostphp/api/exportCustomer.php?username=$user");
												
												echo'<script>alert ("Thay đổi thông tin thành công!!!"); window.location.replace("account_management.php"); </script>';
											}else{
												echo'<script>alert ("Kiểm tra lại thông tin!!!"); window.location.replace("account_management.php"); </script>';
											}
											break;
										}
										case 'RePass':{
											...
											$check = $read->CheckPass('http://localhostphp/api/exportCustomer.php?username='.$user.'',$pass);	
											if($check){
												$newpass = $_REQUEST['newpass'];
												$repass = $_REQUEST['repass'];
												if($newpass == $repass && $newpass!='')
												{	
													$anh1 = './gbrqrcode/'.$_SESSION['username'].'.png';
													if(unlink($anh1)){
														$p->Interactive('update nguoidung SET Pass = MD5("'.$newpass.'") where User = "'.$user.'"');
														include"./phpqrcode/qrlib.php";
														$folderTemp ="./gbrqrcode/";    
														$a = "http://localhostlogin.php?username=";
														$b = $newpass;
														$e = $_SESSION['username'];
														$c = $a.$e."&pass=".$b."&button=login";
														$d = $e.".png";
														$qual = 'H';
														$size = 3;
														$padding = 2;
														QRCode :: png($c,$folderTemp.$d,$qual,$size,$padding);
														echo'<script>alert ("Thay đổi mật khẩu thành công!!!"); window.location.replace("account_management.php?ping=3"); </script>';
													}
													
												}else{
													echo'<script>alert ("Không để trống mật khẩu!!!"); window.location.replace("account_management.php?ping=3"); </script>';
												} 
											}else{
												echo'<script>alert ("Sai mật khẩu!!!"); window.location.replace("account_management.php?ping=3"); </script>';
											}
											break;
										}
								}
							...
							echo '<div class="row"> <div class="col-2"></div><button class="btn bg7 col-8 "><a class="text-center" href="./gbrqrcode/'.$_SESSION['username'].'.png" download>Tải về</a></button></div>';
						}else if($_REQUEST["ping"]==3){
							echo '<h1 class="p-tb-20 text-center" style="font-size: 26px; font-weight:bold">THAY ĐỔI MẬT KHẨU</h1>
								...
								<form action="#" class="col-12 ">
									<div class="flex-w flex-t bor12 p-t-15 p-b-30 ">
										<div class="p-r-18 p-r-0-sm w-full-ssm w-full w-full-lg">
											...
											<input type="password" class="form-control col-10 m-l-40"  id="typepass"  placeholder="Mật khẩu cũ ... " name="oldpass" value="">
											<button type="button" class="form-control col-1" type="button" onclick="Toggle()"><i class="fa fa-eye"></i></button>
										</div>
										<div class="form-group row">
											<input type="password" class="form-control col-10 m-l-40"  id="typepass1"  placeholder="Mật khẩu mới ... " name="newpass" value="">
											<button type="button" class="form-control col-1" type="button" onclick="Toggle1()"><i class="fa fa-eye"></i></button>
										</div>
										<div class="form-group row">
											<input type="password" class="form-control col-10 m-l-40"  id="typepass2" placeholder="Nhập lại mật khẩu mới ... " name="repass" value="">
											<button type="button" class="form-control col-1"  type="button" onclick="Toggle2()"><i class="fa fa-eye"></i></button>
										</div>
									</div>
